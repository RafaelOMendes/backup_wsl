sudo su
code .
ls
cd ..
ks
ls
sudo su
ls
mkdir parallel_programming
cd parallel_programming/
code .
ls[; ls; 
ls
cd parallel_programming/
gcc -o main main;c
gcc -o main main.c
sudo apt install gcc
gcc -o main main.c

gcc -o main main.c
cls
clear
gcc -o main main.c
./main
gcc -o main main.c
./main
gcc -o main main.c
./main
gcc -o main main.c
./main
gcc -o main main.c; ./main
yop
top
quit
ls
git init
git push origin
git push --set-upstream origin master
git add .
git status
git commit -m "primeiro commit"
git config --global user.email "rafa.ouriquemendes@gmail.com"
git config --global user.name "Rafa"
git commit -m "Inicia repositorio WSL"
git remote add origin https://github.com/RafaelOMendes/backup_wsl.git
git branch -M main
git status
git push -u origin main
code .
ls
code .
cd parallel_programming/
code .
ls
cd ..
ls
git status
touth
touch .gitignore
ls
nano .gitignore 
code .
gcc -o main main.c; ./main
cd aula_2
gcc -o main main.c; ./main
]
gcc -o main main.c; ./main
cd ..
cd aula_3
gcc -o main main.c; ./main
gcc -o main exercicio_1.c; ./exercicio_1
gcc -o main exercicio_1.c;
gcc -o main exercicio_1.c; ./exercicio_1
gcc -o exercicio_1 exercicio_1.c; ./exercicio_1
gcc -o main main.c; ./main
gcc -o exercicio_1 exercicio_1.c; ./exercicio_1
gcc -o exercicio_2 exercicio_2.c; ./exercicio_2
gcc -o exercicio_3 exercicio_3.c; ./exercicio_3
gcc -o exercicio_4 exercicio_4.c; ./exercicio_4
gcc -o exercicio_5 exercicio_5.c; ./exercicio_5

gcc -o exercicio_5 exercicio_5.c; ./exercicio_5
gcc -o exercicio_6 exercicio_6.c; ./exercicio_6
cd aula_3
gcc -o exercicio_6 exercicio_6.c; ./exercicio_6
gcc -o exercicio_7 exercicio_7.c; ./exercicio_7
gcc -o exercicio_8 exercicio_8.c; ./exercicio_8
2
gcc -o exercicio_8 exercicio_8.c; ./exercicio_8
gcc -o exercicio_9 exercicio_9.c; ./exercicio_9
gcc -o exercicio_10 exercicio_10.c; ./exercicio_10
sudo npm install -g @google/gemini-cli
gemini auth login
sudo npm install -g @anthropic-ai/claude-code
command -v gemini claude && gemini --version && claude --version
ls
git status
cd pa
cd paperclip/
pnpm dev
claude rc
sudo su
ls
claude rc
sudo su
ls
cd parallel_programming/
ls
code .
cd aula_4/
gcc -o pthread2 pthread2.c -lpthread; ./pthread2
gcc -o pthread2s pthread2s.c -lpthread; ./pthread2s
gcc -o pthreads2 pthreads2.c -lpthread; ./pthreads2
clear
gcc -o pthreads2 pthreads2.c -lpthread; ./pthreads2
gcc -o pthreads3 pthreads3.c -lpthread; ./pthreads3
cd aula_5
gcc -o openmp_2 openmp_2.c -fopenmp
gcc -o openmp_2 openmp_2.c -fopenmp; openmp_2
gcc -o openmp_2 openmp_2.c -fopenmp; ./openmp_2
clear
gcc -o openmp_2 openmp_2.c -fopenmp; ./openmp_2
clear
gcc -o openmp_2 openmp_2.c -fopenmp; ./openmp_2
clear
gcc -o openmp_2 openmp_2.c -fopenmp; ./openmp_2
clear
gcc -o openmp_3 openmp_3.c -fopenmp; ./openmp_3
clear
gcc -o openmp_3 openmp_3.c -fopenmp; ./openmp_3
clear
gcc -o openmp_3 openmp_3.c -fopenmp; ./openmp_3
clear
gcc -o openmp_3 openmp_3.c -fopenmp; ./openmp_3
clear
gcc -o openmp_4 openmp_4.c -fopenmp; ./openmp_4
clear
gcc -o openmp_4 openmp_4.c -fopenmp; ./openmp_4
clear
gcc -o openmp_4 openmp_4.c -fopenmp; ./openmp_4
clear
gcc -o openmp_4 openmp_4.c -fopenmp; ./openmp_4
cd aula_4
gcc -o usofork usofork.c; ./usofork
gcc -o usofork uso_fork.c; ./usofork
